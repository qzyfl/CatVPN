...
build_panel() {
    ...
    ensure_swap
    ensure_go
    is_zh && step 3 6 "编译 CatVPN 定制面板" || step 3 6 "Building CatVPN panel"
    cd "$SRC_DIR"
    # X-MILI 仓库根带的 pow_alps.c 是未被任何 Go 引用的游离 C 文件, 不删会让 go build 报
    # "C source files not allowed when not using cgo"; 删除后才能编译。
    rm -f pow_alps.c
    export PATH="$PATH:/usr/local/go/bin"
    export GOPROXY="https://goproxy.cn,direct"
    export CGO_ENABLED=1
    export GOMAXPROCS=1   # 串行编译, 压住小内存盒单点峰值内存, 防 OOM
    mkdir -p "$INSTALL_DIR"
