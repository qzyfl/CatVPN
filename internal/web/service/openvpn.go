...
// managedOutboundMu guards the managed "vpngate" outbound injection into the
// Xray template config so concurrent connect/cancel/recovery calls do not race.
var managedOutboundMu sync.Mutex

// xrayReferencesOutbound reports whether the given outbound tag is referenced by
// any routing rule or balancer in the parsed Xray config map.
func xrayReferencesOutbound(configMap map[string]any, tag string) bool {
	...
	for _, r := range rules {
		...
		if rule["outboundTag"] == tag || rule["balancerTag"] == tag {
			return true
		}
	}
	return false
